...
Select concat(nombre,' ', apellido1,' ', apellido2) as nombreEmpleado, email,
CodigoJefe from Empleados where CodigoJefe='7';
4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa.
SELECT nombre, apellido1, apellido2, puesto from empleados where
codigoempleado=1;
5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que
no sean representantes de ventas.
SELECT puesto, nombre, apellido1, Apellido2 FROM `empleados` WHERE
...
SELECT CodigoCliente,FechaPago,Cantidad FROM Pagos where
FormaPago='Paypal' and FechaPago like '2009-%' order by Cantidad desc;
...
SELECT ciudad, codigoEmpleadoRepVentas FROM clientes WHERE
ciudad='Madrid' and (codigoEmpleadoRepVentas=11 or
codigoEmpleadoRepVentas=30)
Manejando subconsultas
1. Devuelve el producto que más unidades tiene en stock.
SELECT * FROM `productos` WHERE CantidadEnStock= (SELECT
MAX(CantidadEnStock) FROM productos);
2. Obtén un listado con el nombre de cada cliente y el nombre y apellido de su
...
3. Muestra el nombre de los clientes que hayan realizado pagos junto con el
...
4. Muestra el nombre de los clientes que no hayan realizado pagos junto con el
nombre de sus representantes de ventas.
SELECT clientes.NombreContacto, concat(empleados.Nombre,
empleados.Apellido1) as representante FROM `clientes`
INNER JOIN empleados on
...
pagos);
5. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus
...
6. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus
representantes junto con la ciudad de la oficina a la que pertenece el representante.
...
8. Devuelve el nombre de los clientes y el nombre de sus representantes junto con la
...
9. Devuelve un listado con el nombre de los empleados junto con el nombre de sus
jefes.
...
10. Devuelve un listado que muestre el nombre de cada empleados, el nombre de su
...
12. Devuelve un listado de las diferentes gamas de producto que ha comprado cada
cliente.
...
14. Devuelve el nombre, los apellidos y el email de los empleados que están a
...
SELECT * FROM Productos WHERE PrecioVenta = (select max(PrecioVenta) from
Productos);
